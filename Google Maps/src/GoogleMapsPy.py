...
class Polygon(object):
	...
	def strokeOpacity():
	    doc = "The strokeOpacity property."
	    def fget(self):
...
class GoogleMapsPy(object):
	...
	def __getJs(self):
		...
		""" % (self.__zoom, self.__mapTypeId, self.__center.latitude, self.__center.longitude)
		...
		if self.__polygonCount > 0:
			...
			for polygon in self.__polygonList:
				...
				""" % (i,i,polygon.fillColor,polygon.fillOpacity,polygon.strokeColor,polygon.strokeOpacity,polygon.strokeWeight,i)
				i += 1
		...
		if self.__showPath:
			...
			self.__jsCode += """
				...
				poly.setEditable(%s);
				poly.setMap(map);
			""" % (self.__polyLine.strokeColor, self.__polyLine.strokeWeight, self.__polyLine.strokeOpacity, str(self.__polyLine.editable).lower())
